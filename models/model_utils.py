...
def kernel_combination(R, sim_mat, gamma, alpha, mask):
    dnorm2 = norm_sq(R, R)
    rbf = (-gamma * dnorm2).exp()
    K = alpha * sim_mat + (1.0 - alpha) * rbf
    K = mask * K + (1.0 - mask) * sim_mat
    return K
    
    
def rls_train(R, sim_mat, K, sigma, mask, device):
    Y = torch.mm(R, sim_mat)
    Y = (Y - Y.min(0)[0].unsqueeze(0))/(Y.max(0)[0].unsqueeze(0) - Y.min(0)[0].unsqueeze(0))
    Y = (1-mask)*Y
    Y = R * mask + Y
    vec = (K + sigma*torch.eye(K.size()[0]).to(device)).inverse()
    return torch.mm(torch.mm(K, vec), Y)
